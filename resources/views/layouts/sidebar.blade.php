<aside class="sidebar p-4">
    ...
    <nav class="nav flex-column gap-1 mb-auto">
        <a href="{{ route('dashboard') }}" class="nav-link d-flex align-items-center gap-3 py-2 px-3 rounded-3 {{ request()->routeIs('dashboard') ? 'active-sidebar' : 'text-muted hover-sidebar' }}">
            <i class="bi bi-grid-fill"></i>
            <span class="fw-bold small">Dashboard</span>
        </a>

        <a href="/events" class="nav-link d-flex align-items-center gap-3 py-2 px-3 rounded-3 {{ request()->is('events*') ? 'active-sidebar' : 'text-muted hover-sidebar' }}">
            <i class="bi bi-calendar-event"></i>
            <span class="fw-bold small">Events</span>
        </a>

        <a href="/tasks" class="nav-link d-flex align-items-center gap-3 py-2 px-3 rounded-3 {{ request()->is('tasks*') ? 'active-sidebar' : 'text-muted hover-sidebar' }}">
            <i class="bi bi-check2-square"></i>
            <span class="fw-bold small">Tasks</span>
        </a>

        <a href="/finance" class="nav-link d-flex align-items-center gap-3 py-2 px-3 rounded-3 {{ request()->is('finance*') ? 'active-sidebar' : 'text-muted hover-sidebar' }}">
        ...
        <a href="/organizations" class="nav-link d-flex align-items-center gap-3 py-2 px-3 rounded-3 {{ request()->is('organizations*') ? 'active-sidebar' : 'text-muted hover-sidebar' }}">
    ...
    <div class="mt-4 pt-4 border-top border-light">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="nav-link d-flex align-items-center gap-3 py-2 px-3 rounded-3 text-danger w-100 border-0 bg-transparent hover-danger-sidebar transition-all">
